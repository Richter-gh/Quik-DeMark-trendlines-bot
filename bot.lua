...
function NewInit()
    ...
    if state[1]==nil then 
        if not existsFile(getScriptPath().."\\state_fut.lua") then
            for i=1,#tickers do           	            
            ...
            assert( table.save( state, getScriptPath().."\\state_fut.lua" ) == nil )
        else
            for i=1,#tickers do           	            
                state[tickers[i]]={name=tickers[i].name,uptrend=false,uptrendvalue=0,upwork=false,downwork=false,downtrend=false,downtrendvalue=0,
                                    linebreakup=false,linebreakdown=false,curlineup={},prevlineup={},curlinedown={},
            ...
            local tmp           
            tmp,err = table.load(getScriptPath().."\\state_fut.lua" )
            assert( err == nil ) 
            for i=1,#tickers do 
                if tmp[tickers[i]]~=nil then
                    state[tickers[i]]=tmp[tickers[i]]
                end
            end    
        end
    end
    if pos[1]==nil then 
        if not existsFile(getScriptPath().."\\pos_fut.lua") then
            for i=1,#tickers do    	        
	            pos[tickers[i]]={p='n',targetlong=0,targetshort=0,stoplong=0,stopshort=0,quallong=0,qualshort=0,tp=false}                
            end
            assert( table.save( pos, getScriptPath().."\\pos_fut.lua" ) == nil )
        else
            for i=1,#tickers do           	            
                pos[tickers[i]]={p='n',targetlong=0,targetshort=0,stoplong=0,stopshort=0,quallong=0,qualshort=0,tp=false}
            end 
            local tmp           
            tmp,err = table.load(getScriptPath().."\\pos_fut.lua" )
            assert( err == nil ) 
            for i=1,#tickers do 
                if tmp[tickers[i]]~=nil then
                    pos[tickers[i]]=tmp[tickers[i]]
                end
            end  
        end
    end    
    if stats[1]==nil then       
	    if not existsFile(getScriptPath().."\\stats_fut.lua") then
	        for i=1,#tickers do
    	    ...
    	    local l = io.open(getScriptPath().."\\stats_fut.lua", "w")       
            l:close() 
            assert( table.save( stats, getScriptPath().."\\stats_fut.lua" ) == nil )
        else
            for i=1,#tickers do
                if not existsFile(getScriptPath().."\\Logs\\Stats\\"..tickers[i].."_gudlog.txt") then
                    local l = io.open(getScriptPath().."\\Logs\\Stats\\"..tickers[i].."_gudlog.txt", "w")       
            ...
            local tmp           
            tmp,err = table.load(getScriptPath().."\\stats_fut.lua" )
            assert( err == nil ) 
            for i=1,#tickers do 
                if tmp[tickers[i]]~=nil then
                    stats[tickers[i]]=tmp[tickers[i]]
                end
            end
        end
    end     
...
function Long(ticker)        
    ...
    if settings[ticker].tp~=0 and tpvalue>settings[ticker].tpmax then
        ...
        pos[ticker].stoplong=SysFunc.toPrice(settings[ticker].name,downcur-(settings[ticker].tp)*settings[ticker].stop)
...
function Short(ticker)
    ...
    if settings[ticker].tp~=0 and tpvalue>settings[ticker].tpmax then
        ...
        pos[ticker].stopshort=SysFunc.toPrice(settings[ticker].name,upcur+(settings[ticker].tp)*settings[ticker].stop)
